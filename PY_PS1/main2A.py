...
# *** NEW: Import the tabulate library ***
try:
    from tabulate import tabulate
except ImportError:
    # A cleaner error handler than a standard crash
...
# --- Helper Function for Table Display (New) ---
def display_results_table(data: List[Dict[str, Any]]):
    ...
    """
    if not data:
        print_colored("No user data collected to display.", ConsoleColors.YELLOW)
        return

    print_colored("\n--- Active Directory User Search Results ---", ConsoleColors.BLUE)
    
    # Define headers and the order of keys for the table
    # Note: DistinguishedName is excluded from the *table* to keep it readable, 
    # but it remains in the JSON output for completeness.
    headers = [
        "Name", 
        "sAMAccountName", 
        "UserPrincipalName", 
    ...
    # We use a list comprehension to pull the values in the correct order
    table_data = [
        [
            user.get("Name", "N/A"),
            user.get("SamAccountName", "N/A"),
            user.get("UserPrincipalName", "N/A"),
            # Format the boolean 'IsDisabled' for better readability in the table
...
def search_ad_users(search_name: str, dc_ip: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print_colored(f"  --> Executing PowerShell command for '{search_name}'...", ConsoleColors.DARK_GRAY)
...
def main():
    ...
    for name in name_array:
        ...
        if not users:
            ...
            # *** MODIFIED: The per-user print loop is removed to rely on tabulate for final output. ***
            for user in users:
                ...
                # The keys here MUST match the headers/keys used in display_results_table
                user_object = {
    ...
    # 6. Final JSON Conversion and Display (Original logic preserved)
    print_colored("\n=========================================================", ConsoleColors.WHITE)
